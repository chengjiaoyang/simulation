...
JTopo.Link.prototype.drawanimepic=function(imgurl,scene,anime,width,height){
    var imgnode=new JTopo.Node();
    anime=anime?anime:100
...
function create_element( child , left , top , id ,data,formdata ,netdata ){
    ...
    net_element_ob.setImage("../images/" + all_list[child.id][1]);
...
//removeHandler
    ...
    //        currentLine = this;
    //        link_obj.handlerLine(e);
    //    });
    //},
    //new_link:function(nodeA, nodeZ){
...
var Monitor_flow = {
    ...
    run: function(){
        ...
        $.getJSON('http://10.10.1.67:8080/net/index.php/topo_manage/topo_manage/get_link_flow?callback=?',data,function(msg){
...
(function(){
    $("#start").click(function () {
        ...
        if(stt){
            $.getJSON('http://10.10.1.67:8080/net/index.php/topo_manage/topo_manage/topo_run?callback=?',data,function(msg){
                if( msg && msg.retCode == 0 ){
                    flux.start_flux();
                    //$('#target').addClass('topogo');
                    ...
                    alert( '开启失败：'+msg.retMsg )
                    Monitor_flow.close();
                }
            })
        }
        else {
            $.getJSON('http://10.10.1.67:8080/net/index.php/topo_manage/topo_manage/topo_stop?callback=?',data,function(msg){
                if( msg && msg.retCode == 0 ){
